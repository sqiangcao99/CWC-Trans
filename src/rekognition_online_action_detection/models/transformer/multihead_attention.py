...
class DotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, res_pos = None, attn_mask=None):
        attn_output_weights = torch.bmm(q, k.transpose(1, 2))
        if res_pos != None:
            nbh, s1,s = attn_output_weights.shape 
            nh = res_pos.shape[0]
            attn_output_weights = attn_output_weights.view(-1, nh, s1, s)
            attn_output_weights = attn_output_weights + res_pos.unsqueeze(0)
            attn_output_weights = attn_output_weights.view(nbh, s1, s)
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.0, bias=True, qdim=None, kdim=None):
    ...
    def forward(self, q, k, v, res_pos=None,attn_mask=None, key_padding_mask=None):
        ...
        attn_output = self.dotproductattention(q, k, v, res_pos = res_pos, attn_mask = mask)
...
class MultiheadAttentionStream(MultiheadAttention):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.0, bias=True, qdim=None, kdim=None):
        super(MultiheadAttentionStream, self).__init__(embed_dim, num_heads, dropout, bias, qdim, kdim)
