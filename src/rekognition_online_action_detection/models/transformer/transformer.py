...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, num_tokens=None, dim_feedforward=2048, dropout=0.1, activation='relu', relative=False):
        super(TransformerEncoderLayer, self).__init__()

        self.self_attn = MultiheadAttention(d_model, nhead, dropout=dropout, qdim=d_model, kdim=d_model)
        ...
        if self.relative:
            self.relative_position_bias_table = nn.Parameter(torch.zeros((2 * num_tokens - 1) , nhead))


        ## Generate table index
            coords = torch.arange(num_tokens)
            relative_coords = coords[:, None] - coords[None, :]
            relative_coords += num_tokens - 1
    ...
    def _init_parameters(self):
        if self.relative:
            with torch.no_grad():
                self.relative_position_bias_table.normal_(0.0, 0.02).clamp_(-2.0, 2.0)
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        if self.relative:
            relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(-1, self.num_tokens, self.num_tokens)
        else:
            relative_position_bias = None
        src2 = self.self_attn(src, src, src, res_pos = relative_position_bias, attn_mask=src_mask,
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None,
        ...
        tgt = tgt + self.dropout1(tgt2)
        tgt = self.norm1(tgt)
        tgt2 = self.multihead_attn(tgt, memory, memory, res_pos=None, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)[0]
        tgt = tgt + self.dropout2(tgt2)
        tgt = self.norm2(tgt)
        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt))))
